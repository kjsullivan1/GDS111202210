...
function shoot(){
    ...
    //condtional statement --> checks for TRUTH; uses relational (conditional) operators
    if (currentAmmo > 0){
...
//functions can also be PASSED VALUES; these values are called PARAMETERS (a,b) and mst be sent to the function during the call in order for the function to work (func relies on this data to do its job!)
...
function reload() {
    ...
    //find the value of differnce by calling getDiff() and sending ammo values
    ...
    if (totalAmmo >= difference) {
        ...
        currentAmmo += difference //currentAmmo = curentAmmo + differnce
        totalAmmo -= difference  //totalAmmo = totalAmmo - differnce
    }
